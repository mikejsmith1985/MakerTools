...
def apply_texture_to_face(face, texture_key, scale_mm, depth_mm, is_cut=False):
    ...
    # Positive depth = boss (raises out of surface).
    # Negative depth = deboss (cuts into surface).
    ...
    inp = emboss_feats.createInput(profiles_list, [face], depth_vi)
    if inp is None:
        sketch.deleteMe()
        raise RuntimeError('EmbossFeatures.createInput() returned None — the face or profiles may be invalid.')
